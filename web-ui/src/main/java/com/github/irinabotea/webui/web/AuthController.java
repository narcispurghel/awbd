...
public class AuthController {
    ...
    public String login(
        ...
        try {
            BackendDtos.LoginResponse resp = client.login(new BackendDtos.LoginRequest(form.getEmail(), form.getPassword()));
            cookies.writeCookie(response, resp.token(), resp.expiresInSeconds());
            return "redirect:" + safeRedirect;
        } catch (BackendException ex) {
            flash.addFlashAttribute("error", ex.isUnauthorized() ? "Invalid email or password" : ex.getMessage());
    ...
    public String register(
        ...
        try {
            client.register(new BackendDtos.RegisterRequest(
