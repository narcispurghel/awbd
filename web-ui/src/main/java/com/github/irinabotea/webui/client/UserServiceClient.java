...
public class UserServiceClient {
    ...
    private <T> T post(String path, Object body, Class<T> type, boolean withAuth) {
        try {
            T result = http.post()
                ...
                .headers(h -> { if (withAuth) addAuth(h); })
                .body(body)
                .retrieve()
                .body(type);
            return require(result);
    ...
    private BackendException translate(RestClientResponseException ex) {
        ...
        @Nullable String msg = body == null ? ex.getMessage() : body.message();
        @Nullable List<BackendDtos.FieldError> fields = body == null ? null : body.fieldErrors();
